...
OWNER_ID = 5270790672
BOT_USERNAME = "YourBotUsername" # ضع هنا يوزر بوتك
...
LANGUAGES = {"AR": "العربية 🇮🇶", "EN": "English 🇺🇸", "FR": "Français 🇫🇷"} # اختصار القائمة للوضوح

def get_settings(chat_id):
    if chat_id not in group_settings:
        group_settings[chat_id] = {
            'link_filter': True, 'spam_filter': True, 
            'warn_limit': 3, 'action': 'kick' # kick, mute, delete
        }
...
async def monitor(message: types.Message):
    ...
    if violation:
        user_warnings[message.from_user.id] += 1
        if user_warnings[message.from_user.id] >= s['warn_limit']:
            if s['action'] == 'kick': await bot.ban_chat_member(chat_id, message.from_user.id)
            elif s['action'] == 'mute': await bot.restrict_chat_member(chat_id, message.from_user.id, permissions=ChatPermissions(can_send_messages=False))
            user_warnings[message.from_user.id] = 0
        else:
            await message.delete()
            await message.answer(f"⚠️ Warning {user_warnings[message.from_user.id]}/{s['warn_limit']}")
